...
function startAutoSync() {
    ...
    setInterval(() => {
        ...
        let b = currentUser.boosts.gather || 1; if(currentUser.hero==='miner') b *= 1.5;
...
async function siegeCastle() { const d=await apiCall({action:'siegeCastle',username:currentUser.name}); if(d.success){currentUser={name:currentUser.name,...d.user};updateUI(); if(d.castleOwner) document.getElementById('castle-owner').innerText = d.castleOwner;}else alert(d.error); }
...
async function depositFaucetPay() {
    const FP_MERCHANT_EMAIL = 'ВАШ_EMAIL_ОТ_FAUCETPAY@gmail.com'; // ЗАМЕНИТЕ!
    ...
    const depositLink = `https://faucetpay.io/checkout?` + new URLSearchParams({
        ...
        currency: 'DOGE', amount: 1, callback: callbackUrl, custom_username: currentUser.name
    }).toString();
...
function updateUI() {
    ...
    document.getElementById('hero-name').innerText = currentUser.hero ? (currentUser.hero === 'miner' ? '🧙 Шахтер (+50% ресов)' : '⚔️ Генерал (+20% армии)') : 'Нет';
    document.getElementById('forge-lvl').innerText = currentUser.forgeLvl || 0;
