...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (action === 'login') {
            ...
            let mapBonus = 1 + (world.map.filter(p => p.owner === username).reduce((acc, p) => acc + p.bonus, 0));
            ...
            u.resources.wood += u.buildings.woodcutter * 2 * elapsed * g * ((u.building_hp.woodcutter||0) / (BUILDING_COSTS.woodcutter.hp * u.buildings.woodcutter || 1));
            u.resources.iron += u.buildings.mine * 1 * elapsed * g * ((u.building_hp.mine||0) / (BUILDING_COSTS.mine.hp * u.buildings.mine || 1));
            u.resources.stone += u.buildings.mine * 1 * elapsed * g * ((u.building_hp.mine||0) / (BUILDING_COSTS.mine.hp * u.buildings.mine || 1));
            u.resources.food += u.buildings.farm * 5 * elapsed * g * ((u.building_hp.farm||0) / (BUILDING_COSTS.farm.hp * u.buildings.farm || 1));
            u.resources.mana += u.buildings.tower * 0.5 * elapsed * g * ((u.building_hp.tower||0) / (BUILDING_COSTS.tower.hp * u.buildings.tower || 1));
            u.last_update = now; 
            ...
            await sbFetch('users', 'PATCH', { resources: u.resources, last_update: u.last_update, buildings: u.buildings, building_hp: u.building_hp, construction: u.construction, expedition: u.expedition, rings: u.rings }, `?username=eq.${username}`);
        ...
        // Автодобыча
        let elapsed = (now - u.last_update) / 1000;
        let mapBonus = 1 + (world.map.filter(p => p.owner === username).reduce((acc, p) => acc + p.bonus, 0));
        let heroGatherBonus = u.hero === 'miner' ? 1.5 : 1;
        let ringBonus = 1 + (u.rings.gather * 0.05);
        let chaosBonus = (world.chaos_event_end > now) ? 2 : 1;
        ...
        if (action === 'upgrade') { const b=req.body.building;if(u.construction)return res.json({success:false,error:'Строится!'});const c=BUILDING_COSTS[b];if(!c)return res.json({success:false,error:'Не найдено'});if(u.buildings.townhall<c.req)return res.json({success:false,error:`Ратуша ${c.req} ур!`});const m=Math.pow(1.5,u.buildings[b]);for(let r in c)if(r!=='time'&&r!=='hp'&&r!=='req'&&(u.resources[r]||0)<c[r]*m)return res.json({success:false,error:`Мало ${r}!`});for(let r in c)if(r!=='time'&&r!=='hp'&&r!=='req')u.resources[r]-=c[r]*m;u.construction={building:b,finishTime:now+((c.time*m)/(u.boosts.build||1)*1000)}; updateData = {...updateData, construction: u.construction}; }
        ...
        else if (action === 'buyHero') { const h=req.body.hero; const cost = 50; if(u.balance < cost) return res.json({success:false,error:'Нужно 50 GRC'}); if(u.hero) return res.json({success:false,error:'Уже есть герой'}); u.balance -= cost; u.hero = h; updateData = {...updateData, balance: u.balance, hero: u.hero}; }
        else if (action === 'buyShield') { const cost = 20; if(u.balance < cost) return res.json({success:false,error:'Нужно 20 GRC'}); u.balance -= cost; u.shield = now + 86400000; updateData = {...updateData, balance: u.balance, shield: u.shield}; }
        else if (action === 'blackMarket') { const t=req.body.type; const cost=100; if(u.balance<cost)return res.json({success:false,error:'Нужно 100 GRC'}); u.balance-=cost; if(t==='wood')u.resources.wood+=10000; else if(t==='iron')u.resources.iron+=5000; else if(t==='stone')u.resources.stone+=5000; updateData = {...updateData, balance: u.balance}; }
        else if (action === 'buyRing') { const t=req.body.type; const cost=200; if(u.balance<cost)return res.json({success:false,error:'Нужно 200 GRC'}); if(u.rings[t]>=5)return res.json({success:false,error:'Максимум 5 колец!'}); u.balance-=cost; u.rings[t]++; updateData = {...updateData, balance: u.balance, rings: u.rings}; }
        else if (action === 'triggerChaos') { const cost=500; if(u.balance<cost)return res.json({success:false,error:'Нужно 500 GRC'}); u.balance-=cost; world.chaos_event_end=now+3600000; await sbFetch('world', 'PATCH', { chaos_event_end: world.chaos_event_end }, `?id=eq.main`); updateData = {...updateData, balance: u.balance}; }
        else if (action === 'siegeCastle') { let ap=u.army.warriors*10+u.army.archers*15+u.army.cavalry*25; if(ap < 5000) return res.json({success:false,error:'Нужно 5000 силы!'}); if(world.castle_owner === username) return res.json({success:false,error:'Вы уже Владыка!'}); world.castle_owner = username; u.army.warriors = Math.ceil(u.army.warriors * 0.5); u.army.archers = Math.ceil(u.army.archers * 0.5); u.army.cavalry = Math.ceil(u.army.cavalry * 0.5); u.glory += 100; await sbFetch('world', 'PATCH', { castle_owner: world.castle_owner }, `?id=eq.main`); updateData = {...updateData, army: u.army, glory: u.glory}; }
        else if (action === 'raid') { 
            const t=req.body.targetUser;if(t===username)return res.json({success:false,error:'Себя бить нельзя!'});let enArr=await sbFetch('users', 'GET', null, `?username=eq.${t}`); if(!enArr||enArr.length===0)return res.json({success:false,error:'Цель не найдена!'}); let en=enArr[0]; if(en.shield > now) return res.json({success:false,error:'У цели Щит!'}); 
            ...
            await sbFetch('users', 'PATCH', {resources: en.resources, army: en.army, building_hp: en.building_hp}, `?username=eq.${t}`); updateData = {...updateData, army: u.army, glory: u.glory}; } else { u.army={warriors:0,archers:0,cavalry:0}; await sbFetch('users', 'PATCH', {army: {warriors: en.army.warriors+10}, glory: en.glory+10}, `?username=eq.${t}`); updateData = {...updateData, army: u.army}; } 
        }
        else if (action === 'sendExpedition') { let ta=u.army.warriors+u.army.archers+u.army.cavalry;if(ta<50)return res.json({success:false,error:'Нужно 50 юнитов'});if(u.expedition)return res.json({success:false,error:'Уже в экспедиции'});u.expedition={finishTime:now+3600000};u.army.warriors=Math.ceil(u.army.warriors*0.8);u.army.archers=Math.ceil(u.army.archers*0.8);u.army.cavalry=Math.ceil(u.army.cavalry*0.8); updateData = {...updateData, expedition: u.expedition, army: u.army}; }
        else if (action === 'sell') { const r=req.body.resource,a=req.body.amount,p=parseFloat(req.body.pricePerUnit);if(u.resources[r]<a)return res.json({success:false,error:'Мало ресов'});u.resources[r]-=a; await sbFetch('orders', 'POST', {id:Date.now(),seller:username,resource:r,amount:a,price_per_unit:p,total:a*p}); }
        else if (action === 'buy') { const oi=req.body.orderId; const orderArr = await sbFetch('orders', 'GET', null, `?id=eq.${oi}`); if(!orderArr||orderArr.length===0)return res.json({success:false,error:'Ордер не найден'}); const o=orderArr[0]; if(u.balance<o.total)return res.json({success:false,error:'Мало GRC!'}); let tax=o.total*0.1; if(world.castle_owner){const coArr = await sbFetch('users','GET',null,`?username=eq.${world.castle_owner}`); if(coArr.length>0) await sbFetch('users', 'PATCH', {balance: coArr[0].balance + tax}, `?username=eq.${world.castle_owner}`);} u.balance-=o.total; const sArr = await sbFetch('users','GET',null,`?username=eq.${o.seller}`); if(sArr.length>0) await sbFetch('users', 'PATCH', {balance: sArr[0].balance + (o.total-tax)}, `?username=eq.${o.seller}`); u.resources[o.resource]+=o.amount; await sbFetch('orders', 'DELETE', null, `?id=eq.${oi}`); updateData = {...updateData, balance: u.balance}; }
        else if (action === 'getMarket') { const orders = await sbFetch('orders', 'GET'); await sbFetch('users', 'PATCH', updateData, `?username=eq.${username}`); return res.json({ success: true, orders }); }
        else if (action === 'getDepositAddress') {
            try { const fpRes = await axios.get(`https://faucetpay.io/api/v1/getdepositaddress?api_key=${FP_API_KEY}&currency=${FP_CURRENCY}`); if(fpRes.data.status === 200) { await sbFetch('users', 'PATCH', updateData, `?username=eq.${username}`); return res.json({ success: true, address: fpRes.data.deposit_address }); } else return res.json({ success: false, error: 'Ошибка API FP' }); } catch(e) { return res.json({ success: false, error: 'Сервер FP недоступен' }); }
        }
        else if (action === 'withdraw') { const w=req.body.wallet,ga=parseFloat(req.body.grcAmount);if(u.balance<ga)return res.json({success:false,error:'Мало GRC'});const da=ga/WITHDRAW_RATE;if(da<1)return res.json({success:false,error:`Мин ${WITHDRAW_RATE} GRC`});try{const fpRes=await axios.post('https://faucetpay.io/api/v1/send',null,{params:{api_key:FP_API_KEY,amount:Math.floor(da),to:w,currency:FP_CURRENCY}});if(fpRes.data.status===200){u.balance-=ga;updateData.balance = u.balance; await sbFetch('users', 'PATCH', updateData, `?username=eq.${username}`); const ud={...u};delete ud.password_hash;return res.json({success:true,user:ud,dogeSent:Math.floor(da)});}else return res.json({success:false,error:fpRes.data.message});}catch(e){return res.json({success:false,error:'Ошибка сети FP'});} }
        else if (action === 'capturePost') { let p=world.map.find(p=>p.id===req.body.postId);if(!p)return res.json({success:false,error:'Пост не найден'});let pw=u.army.warriors+u.army.archers*2+u.army.cavalry*3;if(pw<50)return res.json({success:false,error:'Мало армии'});p.owner=username;u.army.warriors=Math.ceil(u.army.warriors*0.9);u.army.archers=Math.ceil(u.army.archers*0.9);u.army.cavalry=Math.ceil(u.army.cavalry*0.9); await sbFetch('world', 'PATCH', { map: world.map }, `?id=eq.main`); updateData = {...updateData, army: u.army}; }
        else if (action === 'getLeaderboard') { const users = await sbFetch('users', 'GET', null, `?select=username,glory&order=glory.desc&limit=10`); await sbFetch('users', 'PATCH', updateData, `?username=eq.${username}`); return res.json({ success: true, leaderboard: users }); }
        else if (action === 'sync') { await sbFetch('users', 'PATCH', updateData, `?username=eq.${username}`); const ud = { ...u }; delete ud.password_hash; return res.json({ success: true, user: ud, map: world.map, castleOwner: world.castle_owner, chaosEventEnd: world.chaos_event_end }); }
    ...
    } catch (error) { console.error(error); return res.status(500).json({ success: false, error: 'Server error: ' + error.message }); }
};
