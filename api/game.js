...
module.exports = async (req, res) => {
    ...
    try {
        ...
        // --- FAUCETPAY CALLBACK (ЕДИНСТВЕННЫЙ ИСТОЧНИК GRC ИЗ ВНЕ) ---
        ...
        if (action === 'register') {
            ...
            db.users[username] = {
                passwordHash: hashPassword(password), balance: 0, glory: 0, // GRC = 0!
                ...
                buildingHp: { townhall: 100, woodcutter: 50, mine: 60, farm: 40, barrack: 80, archery: 70, stable: 90, tower: 150 },
        ...
        if (action === 'login') {
            ...
            let g = u.boosts.gather;
            
            u.resources.wood += u.buildings.woodcutter * 2 * elapsed * mapBonus * g * ((u.buildingHp.woodcutter||0) / (BUILDING_COSTS.woodcutter.hp * u.buildings.woodcutter || 1));
            u.resources.iron += u.buildings.mine * 1 * elapsed * mapBonus * g * ((u.buildingHp.mine||0) / (BUILDING_COSTS.mine.hp * u.buildings.mine || 1));
            u.resources.stone += u.buildings.mine * 1 * elapsed * mapBonus * g * ((u.buildingHp.mine||0) / (BUILDING_COSTS.mine.hp * u.buildings.mine || 1));
            u.resources.food += u.buildings.farm * 5 * elapsed * mapBonus * g * ((u.buildingHp.farm||0) / (BUILDING_COSTS.farm.hp * u.buildings.farm || 1));
            u.resources.mana += u.buildings.tower * 0.5 * elapsed * mapBonus * g * ((u.buildingHp.tower||0) / (BUILDING_COSTS.tower.hp * u.buildings.tower || 1));
            u.lastUpdate = now; 
            ...
            if (u.expedition && now >= u.expedition.finishTime) { u.resources.wood += 5000; u.resources.iron += 2000; u.resources.stone += 2000; u.expedition = null; } // Награда только в ресурсах!
        ...
        // PvP НАГРАДЫ ТОЛЬКО В РЕСУРСАХ И СЛАВЕ (GRC НЕ ВЫПАДАЕТ)
        ...
        // P2P РЫНОК (ЕДИНСТВЕННЫЙ СПОСОБ ЗАРАБОТАТЬ GRC ИГРАЯ)
