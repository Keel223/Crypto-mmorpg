...
module.exports = async (req, res) => {
    ...
    // ЗАЩИТА ОТ ЗАГЛАВНЫХ БУКВ: Все логины сохраняются и ищутся маленькими буквами
    ...
    try {
        ...
        if (isGet && action === 'fp_callback') {
            ...
            const uArr = await sbFetch('users', 'GET', null, `?username=eq.${String(custom_username).toLowerCase()}`);
            if (uArr && uArr.length > 0) { 
                const newBal = uArr[0].balance + (parseFloat(amount) * DEPOSIT_RATE);
                await sbFetch('users', 'PATCH', { balance: newBal }, `?username=eq.${String(custom_username).toLowerCase()}`);
        ...
        if (action === 'login') {
            ...
            // БЕЗОПАСНАЯ ПРОВЕРКА: Сервер больше никогда не упадет с ошибкой 500
            if (!uArr || uArr.length === 0) {
        ...
        if (action === 'upgrade') { const b=req.body.building;if(u.construction)return res.json({success:false,error:'Строится!'});const c=BUILDING_COSTS[b];if(!c)return res.json({success:false,error:'Не найдено'});if(u.buildings.townhall<c.req)return res.json({success:false,error:`Ратуша ${c.req} ур!`});const m=Math.pow(1.5,u.buildings[b]);for(let r in c)if(r!=='time'&&r!=='hp'&&r!=='req'&&(u.resources[r]||0)<c[r]*m)return res.json({success:false,error:`Мало ${r}!`});for(let r in c)if(r!=='time'&&r!=='hp'&&r!=='req')u.resources[r]-=c[r]*m;u.construction={building:b,finishTime:now+((c.time*m)/(u.boosts.build||1)*1000)}; updateData = {...updateData, construction: u.construction}; }
            ...
            let t = req.body.targetUser; 
            if(!t) return res.json({success:false,error:'Введите цель'});
            t = String(t).toLowerCase().trim(); // Защита от заглавных
            if(t===username)return res.json({success:false,error:'Себя бить нельзя!'});
            let enArr=await sbFetch('users', 'GET', null, `?username=eq.${t}`); if(!enArr||enArr.length===0)return res.json({success:false,error:'Цель не найдена!'}); let en=enArr[0]; if(en.shield > now) return res.json({success:false,error:'У цели Щит!'}); 
        ...
        else if (action === 'buy') { const oi=req.body.orderId; const orderArr = await sbFetch('orders', 'GET', null, `?id=eq.${oi}`); if(!orderArr||orderArr.length===0)return res.json({success:false,error:'Ордер не найден'}); const o=orderArr[0]; if(u.balance<o.total)return res.json({success:false,error:'Мало GRC!'}); let tax=o.total*0.1; if(world.castle_owner){const coArr = await sbFetch('users','GET',null,`?username=eq.${world.castle_owner.toLowerCase()}`); if(coArr.length>0) await sbFetch('users', 'PATCH', {balance: coArr[0].balance + tax}, `?username=eq.${world.castle_owner.toLowerCase()}`);} u.balance-=o.total; const sArr = await sbFetch('users','GET',null,`?username=eq.${o.seller.toLowerCase()}`); if(sArr.length>0) await sbFetch('users', 'PATCH', {balance: sArr[0].balance + (o.total-tax)}, `?username=eq.${o.seller.toLowerCase()}`); u.resources[o.resource]+=o.amount; await sbFetch('orders', 'DELETE', null, `?id=eq.${oi}`); updateData = {...updateData, balance: u.balance}; }
        ...
        else if (action === 'sync') { await sbFetch('users', 'PATCH', updateData, `?username=eq.${username}`); const ud = { ...u }; delete ud.password_hash; return res.json({ success: true, user: ud, map: world.map, castleOwner: world.castle_owner, chaosEventEnd: world.chaos_event_end || 0 }); }
