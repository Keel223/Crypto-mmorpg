...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (action === 'repair') { const building = req.body.building; const maxHp = BUILDING_COSTS[building].hp * u.buildings[building]; const missingHp = maxHp - (u.buildingHp[building] || maxHp); if(missingHp <= 0) return res.json({success:false, error:'Не нуждается в ремонте'}); const costWood = missingHp * 5; const costStone = missingHp * 5; if(u.resources.wood < costWood || u.resources.stone < costStone) return res.json({success:false, error:'Нужно дерева и камня: ' + costWood}); u.resources.wood -= costWood; u.resources.stone -= costStone; u.buildingHp[building] = maxHp; saveDB(db); const userData={...u}; delete userData.passwordHash; return res.json({ success: true, user: userData }); }
        ...
        if (action === 'getDepositLink') { if (!db.users[username]) return res.json({ success: false, error: 'User not found in DB' }); const callbackUrl = `https://${req.headers.host}/api/game?action=fp_callback&secret=${FP_CALLBACK_SECRET}&custom_username=${username}`; const depositLink = `https://faucetpay.io/checkout?api_key=${FP_API_KEY}&currency=${FP_CURRENCY}&amount=1&callback=${encodeURIComponent(callbackUrl)}&custom_username=${username}`; return res.json({ success: true, link: depositLink }); }
