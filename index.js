...
app.post('/api/register', (req, res) => {
    try {
        ...
        db.users[username] = { passwordHash: crypto.createHash('sha256').update(password).digest('hex'), class: pClass || 'warrior', xp: 0, level: 1, hp: stats.maxHp, maxHp: stats.maxHp, baseDmg: stats.baseDmg, baseDef: stats.baseDef, gold: 50, inventory: [], equippedWeapon: null, equippedArmor: null, inBattle: false, battleState: null };
        const token = crypto.randomUUID(); // Генерируем случайный токен
...
app.post('/api/login', (req, res) => {
    try {
        ...
        if (!user || user.passwordHash !== hash) return res.json({ error: "Неверный логин/пароль" });
...
app.post('/api/sync', auth, (req, res) => {
    try { res.json({ user: safe(db.users[req.username]), market: Object.values(db.market) }); } 
    catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/action', auth, (req, res) => {
    try {
        ...
        if (type === 'explore') {
            ...
            const locs = {
                forest: { mob: "Гоблин", hp: 50, dmg: 8, def: 2, xp: 40, gold: 20, loot: [{n:"Ржавый меч",t:"weapon",p:5,c:0.3}] },
                dungeon: { mob: "Голем", hp: 120, dmg: 15, def: 8, xp: 100, gold: 60, loot: [{n:"Меч Паладина",t:"weapon",p:15,c:0.4}] }
            };
        ...
        if (type === 'battle_turn') {
            ...
            if (action === 'flee') {
                if (Math.random() > 0.5) { user.inBattle = false; user.battleState = null; return res.json({ log: "Вы сбежали!", user: safe(user), battleEnded: true }); }
                else log.push("Побег провалился!");
            } else if (action === 'defend') {
                mob.playerDefTurns = 2; log.push("Вы в обороне.");
            } else { 
                let dmg = Math.max(1, pDmg - mob.def + rand(-3, 5)); mob.currentHp -= dmg; log.push(`Удар: <span style="color:green">${dmg}</span>.`); 
            }
            ...
            if (user.hp <= 0) {
                ...
                log.push(`<span style="color:red">💀 ПОГИБЛИ. -${lost}💰</span>`);
                user.hp = 1; 
                return res.json({ log: log.join('<br>'), user: safe(user), battleEnded: true });
            }
        ...
        if (type === 'use_flask') {
            ...
            return res.json({ log: `+${pot.heal} HP`, user: safe(user) });
        }
        ...
        if (type === 'buy') {
            ...
            return res.json({ log: `Куплено!`, user: safe(user) });
        }

        if (type === 'withdraw') {
            const amount = parseInt(payload.amount);
            if (user.gold < amount || amount < 1000) return res.json({ log: "Мало золота", user: safe(user) });
...
// --- HTML PAGES ---
const CSS = `:root{--bg:#0a0a0a;--p:#141414;--b:#222;--g:#f1c40f;--r:#e74c3c;--bl:#3498db;--gr:#2ecc71;--y:#f39c12;--t:#ddd}*{box-sizing:border-box;margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif}body{background:var(--bg);color:var(--t);height:100vh;display:flex;flex-direction:column;overflow:hidden}.header{background:var(--p);padding:15px;border-bottom:1px solid var(--b);display:flex;justify-content:space-between;align-items:center;z-index:10}.header h1{font-size:16px;color:var(--g)}.content{flex:1;overflow-y:auto;padding:15px;background:#000}.nav{background:var(--p);display:grid;grid-template-columns:repeat(5,1fr);border-top:1px solid var(--b);z-index:10}.nav button{background:none;border:none;color:#888;padding:15px 0;font-size:20px;cursor:pointer}.nav button.active{color:var(--g);background:#1a1a1a;border-top:2px solid var(--g)}.page{display:none;height:100%}.page.active{display:flex;flex-direction:column}.stat-row{display:flex;justify-content:space-between;margin-bottom:5px;font-size:13px;color:#aaa}.stat-row span{color:white;font-weight:bold}.bar{height:6px;background:#222;border-radius:3px;margin-bottom:15px;overflow:hidden}.bar-fill{height:100%;transition:width 0.3s}.hp{background:var(--r)}.xp{background:var(--bl)}.card{background:var(--p);border:1px solid var(--b);padding:12px;border-radius:6px;margin-bottom:10px}.card h3{font-size:14px;margin-bottom:5px;color:white}.card small{color:#888;font-size:12px;display:block;margin-bottom:8px}.log-box{background:#050505;border:1px solid #1a1a1a;padding:10px;border-radius:6px;font-size:13px;line-height:1.5;min-height:150px;max-height:200px;overflow-y:auto;margin-bottom:15px;color:#aaa}.btn{width:100%;padding:14px;border:none;border-radius:6px;font-weight:bold;font-size:14px;cursor:pointer;margin-bottom:8px;color:white;text-transform:uppercase}.btn-atk{background:#27ae60}.btn-def{background:#2980b9}.btn-flask{background:#8e44ad}.btn-flee{background:#555}.btn-sell{background:var(--y);color:#000}.btn:disabled{background:#333!important;color:#666!important;cursor:not-allowed}.input{width:100%;padding:12px;background:#111;border:1px solid var(--b);color:white;border-radius:4px;margin-bottom:8px;font-size:14px}`;
...
function go(p,btn){document.querySelectorAll('.page').forEach(e=>e.classList.remove('active'));document.querySelectorAll('.nav button').forEach(e=>e.classList.remove('active'));document.getElementById('pg-'+p).classList.add('active');if(btn)btn.classList.add('active');if(p==='inv')renderInv();if(p==='mkt')renderMkt();}
async function init(){let d=await r('sync');if(d)updUI(d.user);}
function updUI(u){
    if(!u)return;
    ...
    if(u.inBattle&&u.battleState){document.getElementById('mobN').innerText=u.battleState.mob;document.getElementById('mobHB').style.width=Math.max(0,(u.battleState.currentHp/u.battleState.hp)*100)+'%';if(!battleInterval)startBattleLoop();}
}
async function explore(loc){let d=await r('action',{type:'explore',payload:loc});if(d){document.getElementById('mapLog').innerHTML=d.log;updUI(d.user);if(d.user.inBattle)go('battle',document.querySelectorAll('.nav button')[2]);}}
...
function renderMkt(){r('sync').then(d=>{if(!d)return;let el=document.getElementById('mktC');if(!d.market.length){el.innerHTML='<div class="card" style="text-align:center;color:#666">Пусто</div>';return;}el.innerHTML=d.market.map(m=>{let s=m.item.type==='weapon'?'Урон: +'+m.item.power:'Защита: +'+m.item.power;return '<div class="card"><h3>'+m.item.name+'</h3><small>'+s+' | '+m.seller+'</small><div style="font-size:18px;color:var(--g);margin:10px 0">'+m.price+' 💰</div><button class="btn btn-atk" onclick="buyItem('+m.id+')">КУПИТЬ</button></div>';}).join('');});}
async function buyItem(id){let d=await r('action',{type:'buy',payload:id});if(d){alert(d.log);updUI(d.user);renderMkt();}}
async function wd(){let d=await r('action',{type:'withdraw',payload:{email:document.getElementById('fpE').value,amount:document.getElementById('fpA').value}});if(d)alert(d.log);updUI(d.user);}
init();
</script></body></html>`;
