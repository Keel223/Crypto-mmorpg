...
app.post('/api/register', (req, res) => {
    ...
    db.users[username] = {
        passwordHash: hash(password), class: pClass || 'warrior',
    ...
    res.json({ success: true, user: safe(db.users[username]) });
});

app.post('/api/login', (req, res) => {
    const { username, password } = req.body;
    const user = db.users[username];
    if (!user || user.passwordHash !== hash(password)) return res.json({ error: "Неверный логин/пароль" });
    req.session.user = username;
    res.json({ success: true, user: safe(user) });
});

app.post('/api/check', (req, res) => {
    if (req.session.user && db.users[req.session.user]) return res.json({ ok: true, user: safe(db.users[req.session.user]) });
    res.json({ ok: false });
});
...
app.post('/api/action', (req, res) => {
    const u = req.session.user; if(!u) return res.status(401).json({error:"Auth"});
    const user = db.users[u];
    const { type, target, payload } = req.body;

    if (type === 'fight') {
        if(user.hp <= 0) return res.json({ log: "<span style='color:red'>Вы мертвы! Купите зелье.</span>", user: safe(user) });
    ...
    if (type === 'sell') {
        ...
        db.market.push({ id: Date.now(), seller: u, item: it, price: parseInt(payload) });
        return res.json({ log: "Выставлено на P2P рынок", user: safe(user), market: db.market });
    }

    if (type === 'buy') {
        const lot = db.market.find(m => m.id === target);
        if (!lot || lot.seller === u || user.gold < lot.price) return res.json({ log: "<span style='color:red'>Ошибка покупки</span>", user: safe(user) });
    ...
    if (type === 'withdraw') {
        ...
        // Имитация запроса (ВЕРНОТЬ РАБОТАЕТ С РЕАЛЬНЫМ FAUCETPAY ЕСЛИ ВСТАВИТЬ КЛЮЧ)
        user.gold -= amount; 
        return res.json({ log: `<span style='color:green'>Успешно выведено ${usdt} USDT на ${email}!</span>`, user: safe(user) });
        
        /* РАСКОММЕНТИРОВАТЬ ДЛЯ РЕАЛЬНОГО ВЫВОДА:
        try {
            const r = await axios.post('https://faucetpay.io/api/v1/send', new URLSearchParams({ api_key: FP_KEY, to: email, amount: usdt, currency: 'USDT' }));
            if (r.data.status === 200) { user.gold -= amount; return res.json({ log: `Выплачено ${usdt} USDT!`, user: safe(user) }); }
            else return res.json({ log: r.data.message, user: safe(user) });
        } catch(e) { return res.json({ log: "Ошибка сети", user: safe(user) }); }
        */
    }
});
...
app.post('/api/sync', (req, res) => {
    const u = req.session.user; if(!u) return res.status(401).json({error:"Auth"});
    res.json({ user: safe(db.users[u]), market: db.market });
});
...
const PAGE_AUTH = `
<!DOCTYPE html><html><head><title>Crypto MMO - Вход</title>
<style>
...
<script>
    async function req(u, b={}) { let r = await fetch('/api/'+u, {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(b)}); return r.json(); }
    function e(m) { let el=document.getElementById('err'); el.innerText=m; el.style.display='block'; }
    async function reg() { let d = await req('register', {username: document.getElementById('user').value, password: document.getElementById('pass').value, pClass: document.getElementById('cls').value}); if(d.error) return e(d.error); window.location.href='/game'; }
    async function log() { let d = await req('login', {username: document.getElementById('user').value, password: document.getElementById('pass').value}); if(d.error) return e(d.error); window.location.href='/game'; }
</script>
</body></html>`;

const PAGE_GAME = `
<!DOCTYPE html><html><head><title>Crypto MMO - Игра</title>
<style>
    :root { --bg:#0a0a0a; --p:#151515; --b:#2a2a2a; --g:#f1c40f; --r:#c0392b; --bl:#2980b9; --gr:#27ae60; }
    * { box-sizing: border-box; margin: 0; } body { background: var(--bg); color: #ddd; font-family: Arial; display:flex; justify-content:center; padding:10px; }
    ...
    .logout { position: absolute; top: 20px; right: 20px; color: #666; text-decoration: none; }
...
<div class="app">
    ...
    <div class="mn">
        <div class="lb" id="lg">Добро пожаловать.</div>
        <div class="ct">
...
async function ft(act){a('<i>Действие...</i>');let d=await r('action',{type:'fight',target:loc,payload:act});a(d.log);up(d.user);}
async function sh(){let p=prompt("1. Малое зелье (20💰)\\n2. Большое зелье (50💰)");if(!p)return;let d=await r('action',{type:'shop',target:parseInt(p)-1});a(d.log);up(d.user);}
async function use(id){let d=await r('action',{type:'use',target:id});a(d.log);up(d.user);}
async function eq(id){let d=await r('action',{type:'equip',target:id});a(d.log);up(d.user);}
async function sel(id){let p=prompt("Цена в 💰:");if(!p)return;let d=await r('action',{type:'sell',target:id,payload:p});a(d.log);up(d.user);rm(d.market);}
async function buy(id){let d=await r('action',{type:'buy',target:id});a(d.log);up(d.user);rm(d.market);}
async function wd(){let d=await r('action',{type:'withdraw',payload:{email:document.getElementById('fpE').value,amount:document.getElementById('fpA').value}});a(d.log);up(d.user);}
function up(u){
    if(!u)return;
    document.getElementById('uN').innerText=u.username;
    document.getElementById('uC').innerText=u.class==='mage'?'Маг':'Воин';
    ...
    let xn=u.level*150; document.getElementById('xpT').innerText=u.xp+'/'+xn; document.getElementById('xpB').style.width=(u.xn/u.xn)*100+'%';
    ri(u);
}
function ri(u){
    let el=document.getElementById('iL');
    if(!u.inventory.length){el.innerHTML='<div style="color:#666;grid-column:span 2;">Пусто</div>';return;}
    el.innerHTML=u.inventory.map(i=>{
        let s=i.type==='weapon'?'Урон: +'+i.power:i.type==='armor'?'Защита: +'+i.power:'Лечение: +'+i.heal;
        let b=i.type==='potion'?'<button class="bs bg" onclick="use('+i.id+')">Пить</button>':'<button class="bs bbl" onclick="eq('+i.id+')">Надеть</button> <button class="bs br" onclick="sel('+i.id+')">P2P</button>';
        return '<div class="cd"><h4>'+(i.type==='potion'?'🧪':'⚔️')+i.name+'</h4><small>'+s+'</small><div class="ca">'+b+'</div></div>';
    }).join('');
}
function rm(m){
    let el=document.getElementById('mL');
    if(!m.length){el.innerHTML='<div style="color:#666;grid-column:span 2;">Пусто</div>';return;}
    el.innerHTML=m.map(i=>{
        let s=i.item.type==='weapon'?'Урон: +'+i.item.power:'Защита: +'+i.item.power;
        return '<div class="cd"><h4>⚔️ '+i.item.name+'</h4><small>'+s+'<br>Продавец: '+i.seller+'</small><div style="color:var(--g);font-weight:bold;margin:5px 0;">'+i.price+' 💰</div><div class="ca"><button class="bs bg" onclick="buy('+i.id+')">Купить</button></div></div>';
    }).join('');
}
async function init(){let d=await r('sync');up(d.user);rm(d.market);}
init();
</script>
</body></html>`;
...
// Если человек не авторизован — кидаем на страницу входа
app.get('/', (req, res) => {
    if (req.session.user) res.redirect('/game');
    else res.send(PAGE_AUTH);
});
...
// Страница игры доступна только авторизованным
app.get('/game', (req, res) => {
    if (req.session.user) res.send(PAGE_GAME);
